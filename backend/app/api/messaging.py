...
def conversation_to_out(
    ...
    for participant in conversation.participants:
        name = resolve_participant_name(db, participant.user_type.value, participant.user_id)
        participants.append({
...
def create_direct_conversation(
    ...
    try:
        ...
        logger.info("Extracted from token - user_type: %s, user_id: %s", user_type, user_id)
    except Exception as e:
    ...
    if not daycare_id:
        logger.error("daycare_id not found in token! current_user keys: %s", list(current_user.keys()))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        logger.info("get_or_create_direct_conversation returned successfully. Conversation ID: %s", conversation.id)
        ...
        logger.exception("create_direct_conversation: UNEXPECTED ERROR - Full stack trace:")
...
def get_conversations(
    ...
    return [
        conversation_to_out(db, conv, user_type, user_id)
...
def register_push_token(
    ...
    return {"status": "ok", "token_id": f"{push_token.user_type.value}:{push_token.user_id}:{push_token.token[:20]}..."}
