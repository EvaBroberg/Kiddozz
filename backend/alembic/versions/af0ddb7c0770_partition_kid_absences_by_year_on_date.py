"""partition kid_absences by year on date

Revision ID: af0ddb7c0770
Revises: c2d8ca2d669f
Create Date: 2025-10-22 13:32:24.198742
...
def upgrade() -> None:
    ...
    # Create new partitioned table without constraints first
    op.execute(text("""
        CREATE TABLE kid_absences_new (
            LIKE kid_absences INCLUDING DEFAULTS INCLUDING IDENTITY
        ) PARTITION BY RANGE (date)
    """))
    
    # Drop all existing constraints that were copied
    op.execute(text("""
        ALTER TABLE kid_absences_new DROP CONSTRAINT IF EXISTS kid_absences_pkey;
        ALTER TABLE kid_absences_new DROP CONSTRAINT IF EXISTS uq_kid_absence_kid_date;
        ALTER TABLE kid_absences_new DROP CONSTRAINT IF EXISTS uq_kid_absences_kid_date;
    """))
    
    # Add the primary key that includes the partition key
    op.execute(text("""
    ...
    op.execute(text("""
        ALTER TABLE kid_absences_new ADD CONSTRAINT uq_kid_absences_kid_date UNIQUE (kid_id, date)
    """))
...
def downgrade() -> None:
    ...
    op.execute(text("""
        CREATE TABLE kid_absences_old (
            LIKE kid_absences INCLUDING ALL
        )
    """))
    
    # Restore the original primary key (just id, not id + date)
    op.execute(text("""
        ALTER TABLE kid_absences_old DROP CONSTRAINT kid_absences_old_pkey
    """))
