...
class TestEducatorsApi:
    ...
    def test_search_educators_returns_only_jessica(self, clean_db):
        ...
        try:
            ...
            assert len(educators) == 1, f"Expected exactly 1 educator, got {len(educators)}: {educators}"
            
            # Assert only Jessica is returned
            jessica_result = educators[0]
            assert jessica_result["full_name"] == "Jessica", f"Expected 'Jessica', got '{jessica_result['full_name']}'"
            ...
            assert "Mark Smith" not in mark_names, f"Mark Smith should not appear when searching for Jessica, but got: {mark_names}"
    ...
    def test_search_educators_returns_all_when_no_search(self, clean_db):
        ...
        try:
            ...
            educators = response.json()
            assert len(educators) == 2, f"Expected exactly 2 educators, got {len(educators)}: {educators}"
            
            # Assert both educators are returned
            educator_names = [edu["full_name"] for edu in educators]
            assert "Jessica" in educator_names, f"Jessica should be in results: {educator_names}"
            assert "Mark Smith" in educator_names, f"Mark Smith should be in results: {educator_names}"
